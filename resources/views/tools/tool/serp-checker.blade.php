        ...
        <form action="{{ route('tools.serp_checker')  }}" method="post" enctype="multipart/form-data" id="serp-checker-form">
            ...
            <div class="form-group">
                ...
                <select name="country" id="i-country" class="custom-select{{ $errors->has('country') ? ' is-invalid' : '' }}">
                    ...
                    @foreach(array_diff_key(config('countries'), array_flip(['AX', 'BQ', 'CT', 'NQ', 'FQ', 'GG', 'IM', 'JE', 'JT', 'FX', 'MI', 'ME', 'NT', 'VD', 'PC', 'PZ', 'YD', 'BL', 'MF', 'RS', 'PU', 'SU', 'GB', 'ZZ', 'WK'])) as $key => $value)
                        <option value="{{ $key }}" @if ((old('country') !== null && old('country') == $key) || ('US' == $key && old('country') == null)) selected @endif>{{ $value }}</option>
                    @endforeach
                </select>
